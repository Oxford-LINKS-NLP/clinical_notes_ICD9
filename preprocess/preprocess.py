...
csv_reader = pd.read_csv(args.input_file, chunksize=args.batch_size, usecols=['HADM_ID', 'SUBJECT_ID', 'CATEGORY', 'DESCRIPTION', 'ISERROR','TEXT'], dtype={'HADM_ID': 'str', 'SUBJECT_ID':'int32', 'CATEGORY': 'str', 'DESCRIPTION':'str', 'ISERROR':'str', 'TEXT':'str'}, keep_default_na=False, na_values='')

with open(output_file, 'x') as notes_tokenized_file:

	tokenizer = Tokenizer(args.batch_size, n_cpus, n_threads, MODE)
